...
export default function AdminVehicles() {
  ...
  async function handleEdit() {
    ...
    if (editing._files?.length) {
      ...
      const res = await adminFetch('/api/admin/vehicles.php', { method: 'PUT', body: editing });
      const d   = await res.json();
      setSaving(false);
      if (d.success) { setEditing(null); load(); } else alert(d.message);
    }
  ...
  async function toggleAvailable(v) {
    ...
    const res = await adminFetch('/api/admin/vehicles.php', { method:'PUT', body: { id: v.id, available: v.available ? 0 : 1 } });
    const d   = await res.json();
